...
console.log(fifaData[828]["Stage"]);
...
function getFinals(data) {
    let result = [];
    let dataFinals = data.filter(function(item){
        if (item["Stage"] === "Final"){
            result.push(item);
        } 
    });
...
function getWinnersByYear(getWinners, getYears, data) {
    let finals = getWinners(getFinals, data);
    let years = getYears(getFinals, data);
    for (let i = 0; i < finals.length; i++){
        console.log("In " + years[i] + " " + finals[i] + " won the world cup!" );
    }
};
...
function getAverageGoals(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        homeTeamGoals += data[i]["Home Team Goals"];
        awayTeamGoals += data[i]["Away Team Goals"];
    }
    return [homeTeamGoals / data.length, awayTeamGoals / data.length];
};
